...
```powershell
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File ..\lab.ps1 setup
```

This:

1. Copies `.env.example` → `.env` if missing
2. Pre-pulls all 7 Docker images (saves ~10 min on first `powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File ..\lab.ps1 up`)
...
- **Windows:** `powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File windows-setup.ps1` — verifies Docker Desktop + Compose v2 from PowerShell
